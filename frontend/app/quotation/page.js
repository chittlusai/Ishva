...
export default function QuotationPage() {
  ...
  const handleLeadSubmit = async (e) => {
    ...
    try {
      const res = await fetch('https://formsubmit.co/ajax/vanapalligowtham890@gmail.com', {
        method: "POST",
        headers: { 
        ...
        body: JSON.stringify({
           _subject: "New Pre-Quotation Lead",
           ...leadForm
        })
      });
      const data = await res.json();
      if(data.success === 'false' || data.success === false) {
        setModalConfig({ isOpen: true, message: "FormSubmit Activation Required: Please check your email (vanapalligowtham890@gmail.com) and click 'Activate' so we can forward these leads to you!", type: "error" });
        return;
      }
  ...
  const submitQuotation = async () => {
    try {
      const quoteData = {
        _subject: "New Final Quotation & Payment Request",
        name: leadForm.name,
      ...
      const data = await res.json();
      if(data.success === 'false' || data.success === false) {
         setModalConfig({ isOpen: true, message: "FormSubmit Activation Required: Please check your email (vanapalligowtham890@gmail.com) to activate form receipts before testing the checkout.", type: "error" });
         return;
      }
    } catch(e) { console.error(e) }

    setModalConfig({ isOpen: true, message: "Quotation Sent to Firm! Redirecting to secure payment gateway...", type: "success" });
  };
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 0 && (
              <motion.div key="step0" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="glass-card" style={{ padding: "3rem", borderRadius: 24, textAlign: "center" }}>
                ...
                <form onSubmit={handleLeadSubmit} style={{ display: "flex", flexDirection: "column", gap: 16, textAlign: "left" }}>
                  ...
                  <button type="submit" className="btn-primary" style={{ padding: "1.2rem", marginTop: 16, display: "flex", alignItems: "center", justifyContent: "center", gap: 12, fontSize: "1rem", fontWeight: 600 }}>Start Quotation <ArrowRight size={18} /></button>
                </form>
              </motion.div>
            )}
            ...
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <div className="responsive-grid" style={{ marginBottom: "3rem", gap: "1rem" }}>
                  {SERVICES.map(s => {
            ...
            {step === 5 && (
              <motion.div key="step5" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <button className="btn-outline" onClick={prevStep}><ArrowLeft size={18} /> Back</button>
                  <button className="btn-primary" onClick={submitQuotation} disabled={!paymentMethod} style={{ padding: "1rem 2rem" }}>
